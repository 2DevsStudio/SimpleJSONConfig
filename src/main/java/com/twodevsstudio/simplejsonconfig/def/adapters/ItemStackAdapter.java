...
public class ItemStackAdapter implements JsonSerializer<ItemStack>, JsonDeserializer<ItemStack> {
  ...
  private Map<String, Object> serializeMeta(@NotNull ItemMeta itemMeta) {
    ...
    for (Map.Entry<String, Object> entry : meta.entrySet()) {
      ...
      if (key.equalsIgnoreCase(LORE_MEMBER)) {
        meta.put(key, TextUtility.textComponentAsString(itemMeta.lore()));
        continue;
      }
  ...
  @NotNull
  private Map<String, Object> recursiveSerialization(
      @NotNull ConfigurationSerializable configurationSerializable) {
  ...
  private void deserializeMeta(Map<String, Object> itemData, ItemStack item) {
    ...
    if (meta != null) {
      if (displayName != null) {
        meta.displayName(TextUtility.coloredComponent(displayName));
      }
      if (lore != null) {
        meta.lore(TextUtility.coloredComponent(lore));
      }
    }
  ...
  /**
   * ItemStack deserialization does not support double values in most cases This method recursively
   * converts all double values to integer in original and nested maps
   *
